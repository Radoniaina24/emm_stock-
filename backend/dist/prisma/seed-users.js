...
async function main() {
    ...
    const [deptRows] = await conn.execute("SELECT id, code FROM departments");
    const deptByCode = {};
    for (const d of deptRows)
        deptByCode[d.code] = d.id;
    const [jobRows] = await conn.execute("SELECT id, code FROM job_titles");
    const jobByCode = {};
    for (const j of jobRows)
        jobByCode[j.code] = j.id;
    const [whRows] = await conn.execute("SELECT id FROM warehouses");
    ...
    for (const u of users) {
        ...
        if (!roleId) {
            console.warn(`⚠️  Rôle ${u.roleCode} introuvable`);
            continue;
        }
        const deptId = deptByCode[u.departmentCode];
        if (!deptId) {
            console.warn(`⚠️  Département ${u.departmentCode} introuvable`);
            continue;
        }
        const jobId = jobByCode[u.jobTitleCode];
        if (!jobId) {
            console.warn(`⚠️  Poste ${u.jobTitleCode} introuvable`);
            continue;
        }
        ...
        await conn.execute("INSERT INTO users (id, email, username, password, role_id, status, must_change_password, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 'ACTIVE', ?, NOW(), NOW())", [userId, u.email, u.username, hashed, roleId, false]);
        await conn.execute("INSERT INTO user_profiles (id, user_id, employee_code, first_name, last_name, display_name, department_id, job_title_id, warehouse_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())", [profileId, userId, u.employeeCode, u.firstName, u.lastName, `${u.firstName} ${u.lastName}`, deptId, jobId, whId]);
        created++;
