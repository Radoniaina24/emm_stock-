...
import { useExitQuery } from "@/hooks/use-stock"
...
export function ExitDetailModal({ id, onClose }: Props) {
  const { data, isLoading } = useExitQuery(id ?? "")
      ...
      <ModalPopup className="max-w-2xl">
        ...
        <ModalHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              <ModalTitle>{data ? `Sortie ${data.reference}` : "Sortie"}</ModalTitle>
              <p className="mt-0.5 text-sm text-muted-foreground">
                {data ? `${EXIT_TYPE_LABEL[data.type] ?? data.type} · ${data.warehouse.name}` : "Chargement…"}
              </p>
            </div>
          </div>
        </ModalHeader>
          ...
          {isLoading || !data ? (
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-2 gap-3 rounded-xl border border-border/60 bg-muted/20 p-4 text-sm sm:grid-cols-4">
                ...
                />
                <Info label="Type" value={EXIT_TYPE_LABEL[data.type] ?? data.type} />
                  ...
                  <tbody>
                    {data.lines.map((l) => (
                      <tr key={l.id} className="border-t border-border/60">
                        ...
                        <td className="px-3 py-2 text-right tabular-nums">
                          {Number(l.unitPrice).toLocaleString("fr-FR")}
                        </td>
